...
public class PromocionDAOImp implements PromocionDAO {
	...
	private PrendaDAO prendaDAO = new PrendaDAOImp();
    ...
    public int insertar(Promocion promocion) {
        ...
        Object[] parameters_prendas = new Object[2];
		...
		if(promocion.getPrendas() != null) {
			for(Prenda prendaSel : promocion.getPrendas()){
				parameters_prendas[0] = prendaSel.getIdPrenda();
				parameters_prendas[1] = promocion.getIdPromocion();
				insertado = dbManager.EjecutarProcedimiento("INSERTAR_PRENDA_X_PROMOCION", parameters_prendas,false);
			}
		}
    ...
    public ArrayList<Prenda> listarPrendasAgregadas(int idPromocion) {
        ...
        rs = dbManager.EjecutarProcedimientoLectura("LISTAR_PRENDAS_X_PROMOCION", parameters);
        try {
            while (rs.next()){
                Prenda _prenda = prendaDAO.obtenerPorId(rs.getInt("idPrenda"));
                prendas.add(_prenda);
            }
        } catch (SQLException ex) {
            Logger.getLogger(PromocionDAOImp.class.getName()).log(Level.SEVERE, null, ex);
